...
namespace AdventOfCode2018.Day04
{
    class Tasks
    {
        ...
        public static void Task1and2()
        {
            ...
            int sleepiestMin = guardSleep[sleepiestGuardId].ToList().IndexOf(guardSleep[sleepiestGuardId].Max());
            Console.WriteLine(sleepiestGuardId * sleepiestMin);

            int guardId = guardSleep.Aggregate((l, r) => l.Value.Max() > r.Value.Max() ? l : r).Key;
            int frequentMin = guardSleep[guardId].ToList().IndexOf(guardSleep[guardId].Max());
